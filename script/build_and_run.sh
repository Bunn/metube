...
MODE="${1:-run}"
APP_NAME="MeTube"
BUNDLE_ID="dev.bunn.metube"
BUILD_CONFIGURATION="${METUBE_BUILD_CONFIGURATION:-release}"
...
swift build --package-path "$ROOT_DIR" -c "$BUILD_CONFIGURATION" --product "$APP_NAME"
BUILD_BINARY="$(swift build --package-path "$ROOT_DIR" -c "$BUILD_CONFIGURATION" --show-bin-path)/$APP_NAME"

if [[ "$APP_BUNDLE" != "$ROOT_DIR/dist/MeTube.app" ]]; then
